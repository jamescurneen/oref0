/*
  The provided Node.js script, oref0 autotuning tool, is a part of oref0 (OpenAPS Reference Design). It operates as an autotuning tool for artificial pancreas systems, responsible for adjusting the basal schedule, Insulin Sensitivity Factor (ISF), and Carbohydrate Sensitivity Factor (CSF) based on the output generated by oref0-autotune-prep.js.

  Functionality:
  - Calculates adjustments to the basal schedule, ISF, and CSF based on preprocessed input data.
  - Takes inputs in the form of glucose data, previous autotune data, and pump profile data.
  - Determines the pump profile's use of custom peak time based on user preferences.
  - Generates output with the adjusted values for basal schedule, ISF, and CSF.

  Usage:
  - The script requires the following command-line arguments:
    - <autotune/glucose.json>: Glucose data used for autotuning.
    - <autotune/autotune.json>: Previous autotune data.
    - <settings/profile.json>: Pump profile data.

  Additional Options:
  - The script supports an optional output file argument to write the generated output.

  Note:
  - This script is a vital component of an artificial pancreas system, responsible for continuously adjusting insulin dosages based on analyzed glucose data and previous autotune settings.
*/
