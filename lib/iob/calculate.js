/*
  Insulin On Board (IOB) Calculation Functions

  This JavaScript module provides functions for calculating Insulin On Board (IOB) based on different insulin action curve models. IOB represents the amount of active insulin remaining in the body at a given time after an insulin dose.

  Functions Overview:
  - iobCalc: Determines the appropriate insulin action curve model to use based on user preferences and calls either iobCalcBilinear or iobCalcExponential accordingly.
  - iobCalcBilinear: Computes IOB using a bilinear insulin action curve model, considering insulin duration (dia), time since bolus (minsAgo), and user settings. It calculates both activityContrib (insulin used in the previous minute) and iobContrib (insulin remaining).
  - iobCalcExponential: Computes IOB using an exponential insulin action curve model, considering insulin duration (dia), peak insulin activity time (peak), time since bolus (minsAgo), and user settings. It calculates both activityContrib (insulin used in the previous minute) and iobContrib (insulin remaining).

  Purpose of Functions:
  - These functions aid in diabetes management by estimating the amount of active insulin in the body at a given time, helping users make informed decisions about insulin dosing and treatment adjustments.
  - iobCalc determines the appropriate insulin action curve model (bilinear or exponential) to use based on user settings and calls the respective calculation functions.
  - iobCalcBilinear and iobCalcExponential compute IOB using different mathematical models, allowing flexibility in determining insulin activity based on user profiles and insulin types.

  Note: Accurate IOB calculation is crucial in diabetes management, and these functions facilitate such calculations using different insulin action curve models, considering parameters like duration, peak activity time, and time elapsed since insulin dosing.
*/
