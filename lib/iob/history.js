/*
  This JavaScript module implements functions to calculate temporary treatments (temp basals and boluses)
  for an automated insulin delivery system. It is designed to handle data from a diabetes pump, process
  various events (such as boluses, temp basals, suspends, resumes) and generate temporary treatments
  based on the profile settings and historical data.

  Key Functions:
  - `splitTimespanWithOneSplitter(event, splitter)`: Splits a given event based on a specified splitter.
  - `splitTimespan(event, splitterMoments)`: Splits events based on a set of splitter moments.
  - `splitAroundSuspends(currentEvent, pumpSuspends, firstResumeTime, suspendedPrior, lastSuspendTime, currentlySuspended)`: Splits events around suspends.
  - `calcTempTreatments(inputs, zeroTempDuration)`: Generates temporary treatments considering pump history,
    suspend/resume events, and profile settings. This function produces temporary basal rates and boluses
    based on the insulin delivery system's settings and historical data.

  The code handles suspends, resumes, boluses, and temporary basal rates. It also accounts for overlaps,
  adjusts event lengths, splits events based on basal breakpoints, and calculates temporary boluses
  based on differences in basal rates.

  The `calcTempTreatments` function integrates various other functions and processes pump history data
  to generate an array of temporary treatments. It considers suspends, resumes, profile settings, and
  insulin delivery specifics to compute appropriate temporary treatments for an automated insulin delivery
  system, providing valuable insights into the system's functioning.
*/
